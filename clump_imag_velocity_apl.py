...
#    add_source(ax0,catalog,wcs)
    ...
    v_cmap = mpl.cm.get_cmap('viridis')
    v_cmap = mpl.cm.get_cmap('plasma')
    v_cmap = mpl.cm.get_cmap('cividis')
    v_cmap = mpl.cm.get_cmap('seismic')
    v_cmap = mpl.cm.get_cmap('Reds')
    v_cmap = mpl.cm.get_cmap('coolwarm')
    catalog = load_csv(file_catalog)
    add_source_velo(fig,catalog)
    ...
    for i, struc in enumerate(dendro.trunk):
        if struc.is_leaf:
            ...
            leaf_velo = get_source_velo(leaf_label,catalog)
            color = colors[0]
            subtree = []
            line = 'solid'
            ...
            if leaf_label == 19:
                continue
            fig.show_contour(mask,linewidths=1.5,levels=[0.001,1000],alpha=0.95,filled=True,colors=[v_cmap(norm(leaf_velo))],linestyles=line)#,smooth=1)
        ...
        for j, sub_struc in enumerate(subtree):
            if sub_struc.is_leaf:
                ...
                fig.show_contour(mask,linewidths=1.5,levels=[0.001,1000],alpha=0.95,filled=True,colors=[v_cmap(norm(leaf_velo))])#,smooth=1)
